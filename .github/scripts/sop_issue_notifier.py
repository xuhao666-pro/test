...
STATE_LABELS = {value[1] for value in EVENTS.values() if value[1]}
LABEL_COLORS = {
    "sop-task": "1f6feb",
    "dispatched": "8250df",
...
EVENT_MARKER_RE = re.compile(r"^<!-- sop-notification:event:[0-9a-f]{20} -->$")
...
def channel_delivery_marker(event_marker_value: str, channel: str) -> str:
    """Return a stable per-event marker for an independently retried channel."""

    channel = str(channel or "").strip().lower()
    if not EVENT_MARKER_RE.fullmatch(str(event_marker_value or "")):
        raise NotificationError("Invalid event marker for channel delivery")
    if not CHANNEL_RE.fullmatch(channel):
        raise NotificationError("Invalid notification channel name")
    digest = hashlib.sha256(
        f"{event_marker_value}|{channel}".encode("utf-8")
    ).hexdigest()[:20]
    return f"<!-- sop-notification:channel:{channel}:{digest} -->"
...
def transition_labels(current: list[str], event: str) -> list[str]:
    ...
    if event == "task-dispatched" and any(label in STATE_LABELS for label in current):
        # A repeated dispatch is only an idempotent Issue upsert.  It must not
        # downgrade a later blocked, invalid, pending-validation or valid state.
        return sorted(set(current))
...
def render_event(
    ...
    *,
    ...
    mention = (
        f"@{member_login}"
        if event in {"task-dispatched", "task-reminder"}
        else f"@{coordinator_login}"
    )
    if event == "submission-invalid":
        mention = f"@{member_login} @{coordinator_login}"
    marker = event_marker(assignment_id, event, event_identity or commit_sha)
    lines = [
        marker,
        f"{mention} **{phrase}**",
        "",
    ...
    labels = ["sop-task"] + ([state_label] if state_label else [])
    return {"title": "", "body": "\n".join(lines), "labels": labels, "marker": marker}
...
class GitHubClient:
    ...
    def paginated(self, path: str):
        ...
        page = 1
        while True:
            values = self.request("GET", f"{path}{separator}page={page}")
            if not isinstance(values, list):
                raise NotificationError(f"GitHub API pagination expected a list for {path}")
            yield from values
            if len(values) < 100:
                return
            page += 1
    ...
    def find_issue(self, marker: str) -> dict[str, Any] | None:
        # Labels are a projection and can be removed manually.  The hidden
        # assignment marker is the durable identity, so lookup must not depend
        # on the current label set.
        for issue in self.paginated("/issues?state=all&per_page=100"):
            if marker in str(issue.get("body", "")) and "pull_request" not in issue:
                return issue
        return None

    def comment_exists(self, number: int, marker: str) -> bool:
        comments = self.paginated(f"/issues/{number}/comments?per_page=100")
        return any(marker in str(comment.get("body", "")) for comment in comments)

    def ensure_labels(self, labels: list[str]) -> None:
        existing = {item["name"] for item in self.paginated("/labels?per_page=100")}
        for label in labels:
...
def discover_events(
    ...
    # A branch head can be projected onto many historical records.  A member
    # push only represents the submission manifests changed by that exact
    # push; matching observed_head alone would replay unrelated old tasks.
    changed_manifests = {
        path for path in changed if path.endswith("/submission-manifest.yaml")
    }
    ...
    for stage in dict(stages).values():
        for record in dict(stage.get("records", {})).values():
            if str(record.get("observed_head", "")) != commit_sha:
                continue
            branch = str(record.get("git_branch", "")).strip()
            if not branch or git_ref != f"refs/heads/{branch}":
                continue
            submission_path = str(record.get("submission_path", "")).strip().strip("/")
            if not submission_path:
                continue
            manifest_paths = {
                f"{submission_path}/submission-manifest.yaml",
                f"sop/{submission_path}/submission-manifest.yaml",
            }
            if changed_manifests.isdisjoint(manifest_paths):
                continue
...
def project_state_records(state: dict[str, Any]) -> dict[str, dict[str, Any]]:
    """Flatten trusted stage records by exact assignment ID."""

    records: dict[str, dict[str, Any]] = {}
    stages = dict(state.get("submission_tracking", {})).get("stages", {})
    for stage in dict(stages).values():
        for assignment_id, record in dict(stage.get("records", {})).items():
            if isinstance(record, dict):
                records[str(assignment_id)] = record
    return records


def discover_project_state_events(
    old_state: dict[str, Any],
    new_state: dict[str, Any],
    assignment_paths: dict[str, str],
    assignment_commits: dict[str, str],
    fallback_commit: str,
) -> list[dict[str, str]]:
    """Project remote member transitions from a trusted main state commit.

    Member branches never execute the privileged notification workflow.  The
    main-only state refresher observes them, commits the trusted projection,
    and this function derives the same additive notification events.
    """

    old_records = project_state_records(old_state)
    new_records = project_state_records(new_state)
    events: list[dict[str, str]] = []
    for assignment_id, record in sorted(new_records.items()):
        assignment = assignment_paths.get(assignment_id)
        if not assignment:
            continue
        old = old_records.get(assignment_id, {})
        status = str(record.get("status", ""))
        validation = str(record.get("validation_status", ""))
        submission_commit = str(
            record.get("observed_submission_commit")
            or record.get("observed_head")
            or fallback_commit
        )
        status_changed = status != str(old.get("status", ""))
        validation_changed = validation != str(old.get("validation_status", ""))
        commit_changed = submission_commit != str(
            old.get("observed_submission_commit")
            or old.get("observed_head")
            or ""
        )
        if not (status_changed or validation_changed or commit_changed):
            continue

        # The notifier requires an Assignment Issue before member events.  A
        # repeated task-dispatched event is idempotent and sends no DingTalk
        # message when the Issue already exists.
        events.append(
            {
                "event": "task-dispatched",
                "assignment": assignment,
                "detail": "",
                "commit": assignment_commits.get(assignment_id, fallback_commit),
            }
        )
        if status == "blocked" and status_changed:
            events.append(
                {
                    "event": "task-blocked",
                    "assignment": assignment,
        ...
        if status != "submitted":
            continue
        if status_changed or commit_changed:
            events.append(
                {
        ...
        if validation == "valid" and validation_changed:
            events.append(
                {
                    "event": "submission-valid",
                    "assignment": assignment,
                    "detail": "",
                    "commit": submission_commit,
                }
            )
        elif validation == "invalid" and validation_changed:
            events.append(
                {
                    "event": "submission-invalid",
                    "assignment": assignment,
        ...
        json.dumps(
            {
                ...
                "dingtalk_marker": channel_delivery_marker(
                    rendered["marker"], "dingtalk"
                ),
            }
        )
    )
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--identity",
        default="",
        help="Stable event identity used for idempotency; defaults to commit SHA",
    )
    return parser
