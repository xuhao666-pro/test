...
def update_readme(path: Path, image_path: str, action_url: str = "") -> None:
    action_url = action_url.strip()
    if "\n" in action_url or "\r" in action_url:
        raise RuntimeError("协调操作台 URL 不得包含换行")
    action_block = (
        f"\n[打开协调操作台]({action_url}) · "
        "仅发起受权限和状态前提约束的操作。\n"
        if action_url
    ...
    block = (
        ...
        "## 项目状态看板\n\n"
        f"![项目状态看板]({image_path})\n\n"
        "> 由 SOP 状态自动生成的只读驾驶舱；操作入口不构成任务、提交或 Gate 事实。\n"
        f"{action_block}"
        f"{END_MARKER}"
    )
    if path.exists():
...
def main() -> int:
    ...
    try:
        ...
        update_readme(readme_path, image_path, args.action_url)
        print(
            json.dumps(
                {
                    "svg": str(output_path),
