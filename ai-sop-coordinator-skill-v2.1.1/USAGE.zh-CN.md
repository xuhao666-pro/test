# AI SOP 协调者 Skill V2.1.1 使用说明

V2.1.1 是包装与发行谱系收敛补丁：历史 Coordinator 发行由 Git tag 和清理审计保存，不再要求旧版本根目录常驻。运行时和任务协议不变；A—C 协调 CLI 身份仍为 V1.8.5，D—E 协调 CLI 身份仍为 V2.0.0。

本版本保留 D 正式开发、代码审查与集成、E 发布交付、G4/G5 和灰度闭环，并继续使用统一包模型发现 A—C Member 运行时。一个 Member V2.1 包可以精确承载历史 1.8.0、A—C 1.8.1 和 D—E 2.0.0 三套任务运行时。
