...
def validate_git_workspace(
    ...
    *,
    ...
    if detached_validation:
        ...
        if historical_ancestor.returncode != 0:
            raise SopError(
                "Detached validation commit is not reachable from the exact registered "
                f"member ref {member_ref}: {member_head}"
            )
    ...
    if require_assignment_sync:
        ...
        if not main_snapshot:
            raise SopError(
                f"Member branch contains no authorized common snapshot of {main_ref}"
            )
