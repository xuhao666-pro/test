...
SKILL_VERSION = "1.8.5"
BUILD_ID = "coordinator-cli-1.8.5-unified-member-package-v1"
MEMBER_MINIMUM_VERSION = "1.8.1"
MEMBER_BUILD_ID = "member-cli-1.8.1-assignment-acceptance-v1"
MEMBER_PACKAGE_VERSION = "2.1.0"
MEMBER_PACKAGE_BUILD_ID = "member-package-2.1.0-unified-runtimes-v1"
MEMBER_RUNTIME_PROFILE = "predevelopment"
MEMBER_PREDEVELOPMENT_STAGES = (
    "A",
    "B",
    "C",
)
...
def confirmed_member_release(project: dict[str, Any]) -> dict[str, str]:
    ...
    if control is None:
        ...
        return {
            ...
            "package_version": MEMBER_PACKAGE_VERSION,
            "package_build_id": MEMBER_PACKAGE_BUILD_ID,
            "package_path": f"ai-sop-member-skill-v{MEMBER_PACKAGE_VERSION}",
            "runtime_profile": "legacy_predevelopment",
    ...
    package_fields = ("package_version", "package_build_id", "runtime_profile")
    if any(field in release for field in package_fields):
        required_package_fields = (*package_fields, "protocol_version")
        if any(not str(release.get(field, "")).strip() for field in required_package_fields):
            raise SopError("Confirmed unified Member Skill package identity is incomplete")
        if str(release["package_path"]) != f"ai-sop-member-skill-v{release['package_version']}":
            raise SopError("Confirmed unified Member Skill package path/version mismatch")
...
def _contained_release_file(package: Path, relative: Any, field: str) -> Path:
    value = str(relative or "").strip()
    if not value:
        raise SopError(f"Member runtime release is missing {field}")
    relative_path = Path(value)
    if relative_path.is_absolute():
        raise SopError(f"Member runtime release {field} must be package-relative")
    package_root = package.resolve()
    candidate = (package_root / relative_path).resolve()
    try:
        candidate.relative_to(package_root)
    except ValueError as exc:
        raise SopError(f"Member runtime release {field} escapes its package") from exc
    if not candidate.is_file():
        raise SopError(f"Member runtime release {field} does not exist: {value}")
    return candidate


def _member_release_from_package(package: Path) -> dict[str, str] | None:
    ...
    try:
        ...
        return None
    if not isinstance(manifest, dict) or manifest.get("release_status") != "stable":
        return None
    if manifest.get("package_name") != "ai-sop-member-skill":
        return None
    package_version = str(manifest.get("package_version", "")).strip()
    package_build_id = str(manifest.get("build_id", "")).strip()
    if (
        not package_version
        or not package_build_id
        or re.fullmatch(r"\d+\.\d+\.\d+", package_version) is None
        or package.name != f"ai-sop-member-skill-v{package_version}"
    ):
        return None

    runtime_releases = manifest.get("runtime_releases")
    if isinstance(runtime_releases, dict):
        if str(manifest.get("manifest_schema_version", "")) != "2.1":
            return None
        runtime = runtime_releases.get(MEMBER_RUNTIME_PROFILE)
        if not isinstance(runtime, dict) or runtime.get("release_status") != "stable":
            return None
        version = str(runtime.get("skill_version", "")).strip()
        build_id = str(runtime.get("build_id", "")).strip()
        protocol_version = str(runtime.get("protocol_version", "")).strip()
        project_schema_version = str(runtime.get("project_schema_version", "")).strip()
        stage_ids = runtime.get("stage_ids")
        if not isinstance(stage_ids, list) or tuple(stage_ids) != MEMBER_PREDEVELOPMENT_STAGES:
            return None
        if re.fullmatch(r"\d+\.\d+\.\d+", version) is None:
            return None
        try:
            protocol_path = _contained_release_file(
                package, runtime.get("protocol_path"), "protocol_path"
            )
            cli_path = _contained_release_file(package, runtime.get("cli_path"), "cli_path")
        ...
        if (
            ...
            or not project_schema_version
            or str(protocol.get("skill_version", "")) != version
            or str(protocol.get("build_id", "")) != build_id
            or str(protocol.get("protocol_version", "")) != protocol_version
            or str(protocol.get("project_schema_version", "")) != project_schema_version
            or str(protocol.get("release_status", "")) != "stable"
        ):
            return None
        runtime_profile = MEMBER_RUNTIME_PROFILE
    else:
        # Legacy flat packages used the package identity as the A-C runtime identity.
        ...
        if not (protocol_path.is_file() and cli_path.is_file()):
            return None
        try:
            protocol = load_data(protocol_path)
        except (OSError, ValueError, SopError):
        ...
        if str(protocol.get("skill_version", "")) != version:
            return None
        runtime_profile = "legacy_predevelopment"
...
def discover_latest_stable_member_release(repository_root: Path) -> dict[str, str]:
    ...
    for package in root.glob("ai-sop-member-skill-v*"):
        try:
            package.resolve().relative_to(root)
        except ValueError:
            continue
        release = _member_release_from_package(package)
        if release is not None:
            releases.append(release)
    if not releases:
        raise SopError("No consistent stable ai-sop-member release was found")
    return max(releases, key=lambda item: version_tuple(item["package_version"]))
...
def cmd_init_project(args: argparse.Namespace) -> None:
    ...
    if git_root is not None:
        initial_member_release = discover_latest_stable_member_release(git_root)
        initial_member_release["release_commit"] = run_git(
            git_root, "rev-parse", "HEAD"
        ).stdout.strip()
        initial_confirmation_source = "repository-stable-release"
    else:
        initial_member_release = {
            "name": "ai-sop-member",
        ...
        initial_confirmation_source = "bundled-project-initial-release"
    ...
    dump_data(
        root / "project-state.yaml",
        {
            ...
            "skill_release_control": {
                "status": "confirmed",
                "confirmed_member_skill": initial_member_release,
                "confirmation_source": initial_confirmation_source,
