---
name: ai-sop-member
description: Use when a registered member executes a confirmed V1.8.0 assignment that requires exact Skill release binding, project-controlled AI dialogue collaboration, independent contribution boundaries, and final human owner confirmation.
---
...
1. 定位协调员分发的任务包。
2. 读取 [references/member-workflow.md](references/member-workflow.md) 和 [references/authority-boundaries.md](references/authority-boundaries.md)。
3. 根据 `stage_id` 和 `assignment_kind` 读取 [references/stage-rules.md](references/stage-rules.md)。
4. 创建或校验提交时读取 [references/artifact-contracts.md](references/artifact-contracts.md)。
5. 准备、记录或校验最终人工确认时完整读取 [references/submission-confirmation.md](references/submission-confirmation.md)。
6. V1.8.0 任务完整读取 [references/ai-dialogue-collaboration.md](references/ai-dialogue-collaboration.md)。
7. 使用脚本检查精确 Skill 版本、协议、schema、身份、协作模型、轮次和基线。
...
V1.8.0 任务还会在任何产出创建前核对精确 Member Skill 版本与 `build_id`。`init` 创建辅助 `ai-dialogue-summary.yaml`：`required` 必须完成，`optional` 可由成员明确跳过。该文件只记录结构化过程证据，不替代七类正式产物，也不产生 Gate 效果。
