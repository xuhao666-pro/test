# AI SOP 成员 Skill V2.1.0 使用说明

V2.1.0 是统一发行包，不是新的任务运行时。发行包身份为 `2.1.0 / member-package-2.1.0-unified-runtimes-v1`，包内保留三个精确运行时身份。

| 任务类型 | 运行时身份 | 命令入口 |
| --- | --- | --- |
| 当前 A—C/G1—G3 | `1.8.1 / member-cli-1.8.1-assignment-acceptance-v1` | `ai-sop-member/scripts/member_cli.py` |
| D—E/G4—G5 | `2.0.0 / member-dev-cli-2.0.0-v1` | `ai-sop-member/scripts/development_cli.py` |
| 精确绑定 1.8.0 的历史 A—C 任务 | `1.8.0 / member-cli-1.8.0-ai-dialogue-exact-release-v1` | `ai-sop-member/scripts/member_cli_1_8_0.py` |

任务可以把 `package_path` 指向 `ai-sop-member-skill-v2.1.0`，但 `required_member_skill.version` 与 `build_id` 必须匹配表中的某个运行时。不得使用包版本 2.1.0 替代任务运行时版本。
...
3. Skill 先读取任务的 `required_member_skill`，再按 `package-manifest.json.runtime_releases` 选择唯一匹配的 CLI 和协议资产；包身份与任务运行时身份分开核对。不匹配时停止并显示任务要求、当前候选、指定包、发行提交和经过验证的精确 CLI 命令。
...
python "$env:USERPROFILE\.codex\skills\ai-sop-member\scripts\development_cli.py" --help
python "$env:USERPROFILE\.codex\skills\ai-sop-member\scripts\member_cli_1_8_0.py" --help
```
...
- 所有 1.8.0、1.8.1 和 2.0.0 任务都必须精确匹配任务指定的运行时版本、`build_id` 与协议；更高、较低、同版本不同构建或仅包版本匹配都阻塞。
