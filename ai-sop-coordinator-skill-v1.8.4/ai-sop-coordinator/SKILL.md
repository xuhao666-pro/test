...
description: Use when a coordinator runs the V1.8.4 distributed SOP and must prepare human-readable Gate material, project explicit member acceptance from exact remote refs, validate reachable submission commits, or perform existing coordination and Gate operations.
---
...
本 Skill 是独立协调者发行包，不要求在协调者 Codex 中安装完整 Member Skill。远程提交校验使用自身 `assets/remote-validator/` 中与任务精确版本匹配的校验器；V1.8.2 起从登记分支中解析每个任务自身的可达提交 commit，避免用最新分支 HEAD 回溯误判历史任务。V1.8.3 进一步投影显式任务接受凭证。新任务继续精确绑定仓库中经协调员确认的 Member 版本与 `build_id`。
...
6. 使用脚本管理确定性状态，不手写绕过版本、参与率、责任能力和状态校验。
...
基线冻结后运行 `prepare-skill-release`。系统只推荐元数据、CLI 版本和 `build_id` 一致的 `stable` Member 包；协调员审阅精确预览后使用 Token 运行 `confirm-skill-release`。未确认时阻塞下一阶段新任务派发，已经通过的 Gate 和冻结基线不回滚。新任务自动写入精确版本与 `build_id`，成员启动时强制核对。
