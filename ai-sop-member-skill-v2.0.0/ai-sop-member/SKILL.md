...
> V2.0.0 稳定版。A—C 兼容流程由内置 V1.8.1 CLI 执行；`04-development` 与 `05-release` 成员任务由 V2.0 `development_cli.py` 执行。两套命令使用各自的版本资产，不混用任务契约。

在成员自己的工作区执行一个版本化任务包，并把结果写入该成员专属提交目录。

本 Skill 是独立成员发行包，不包含协调者命令、中央状态写入、汇总、Gate 或看板能力。成员只需安装本包；任务包由使用匹配协议版本的协调者通过 Git 分发。

本草案把成员执行范围扩展到 D 正式开发和 E 发布交付。执行代码、代码审查、发布验证或事故任务前完整读取 [references/development-delivery.md](references/development-delivery.md)；涉及 Git 分支与合并时同时读取 [references/development-git-rules.md](references/development-git-rules.md)。
...
- 在读取任务正文或创建任何产物前，先按 [version-preflight.md](references/version-preflight.md) 比较任务要求与自身 `assets/protocol-version.yaml`。不匹配时只输出结构化版本提醒和可验证的精确 CLI 路径，然后停止；不启动独立启动器，不静默安装、升级、降级或覆盖全局 Skill。
...
9. A—C 使用 `member_cli.py` 检查原有任务契约；D—E 使用 `development_cli.py` 检查 V2.0 精确 Skill、任务接受、代码范围、检查结果、实现 commit 和 owner 确认。
